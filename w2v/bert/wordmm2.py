...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(config.hidden_size*34, 2000),
            nn.ReLU(),
            nn.Linear(2000,2000),
            nn.ReLU(),
            nn.Linear(2000,2000),
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        outputs = self.bert(input_ids,
                            ...
                            head_mask=head_mask)
        pooled_output = outputs[0].view(-1,34*768)
        ...
        if labels is not None:
            loss_fct = MSELoss()
            loss = loss_fct(logits*10000, labels*10000)
...
def read_vector(tokenizer, max_seq_length, model_path='tmp/keywords_aminer_en'):
    model = KeyedVectors.load(model_path)
    x = []
    y = []
    cnt = 0
    for word, vector in zip(model.vocab, model.vectors):
        tokens = ['[CLS]'] + tokenizer.tokenize(word)
...
def train1(model, datax, datay, batch_size, optimizer):
    model.train()
    length = len(datay)
    pbar = tqdm(range(0, length, batch_size))
    for step, i in enumerate(pbar):
        x, y = datax[i:i+batch_size].to(device), datay[i:i+batch_size].to(device)
        optimizer.zero_grad()
        output = model(x[:, 0], attention_mask=x[:, 1], labels=y)
        loss = output[0]
        loss.backward()
        optimizer.step()
...
def test1(model, datax, datay, batch_size):
    model.eval()
    length = len(datay)
    pbar = tqdm(range(0, length, batch_size))
    test_loss = 0
    ...
    with torch.no_grad():
        for step, i in enumerate(pbar):
            ...
            output = model(x[:, 0], attention_mask=x[:, 1], labels=y)
            test_loss += output[0].item()
            max_loss = max(max_loss, output[0].item())
            pbar.set_postfix({'loss': '{:.8f}'.format(test_loss / cnt), 'max': '{:.8f}'.format(max_loss)})
    test_loss /= length
    print('\nTest set: Average loss: {:.8f}, Max loss: {:.8f}\n'.format(test_loss, max_loss))



def sort(x, y):
    tmp = list(range(len(datax)))
    random.shuffle(tmp)
    length = len(y)
    rx = torch.LongTensor(length, 2, 34)
    ry = torch.Tensor(length, 200)
...
if __name__ == '__main__':
    ...
    datax, datay = read_vector(tokenizer, args['max_seq_length'])
    ...
    for epoch in range(args['epochs']):
        ...
        datax, datay = sort(datax, datay)
        train1(model, datax, datay, args['batch_size'], optimizer)
        torch.save(model.state_dict(), 'params1-' + str(epoch) +'.pkl')
        test1(model, datax, datay, args['test_batch_size'])
    ...
    for epoch in range(args['fine_tune_epochs']):
        ...
        datax, datay = sort(datax, datay)
        train1(model, datax, datay, args['batch_size'], optimizer)
        torch.save(model.state_dict(), 'params2-' + str(epoch) +'.pkl')
        test1(model, datax, datay, args['test_batch_size'])
    print('Total Time: ', time.time() - start_time)
