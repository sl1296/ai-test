...
class cbf:
    def on_train_begin(self, x):
        self.cnte = 0
        print('weights init', time.time()-tm)
        ...
        for i in dic:
            j = i.replace(' ','X')
            if(j in xdic):
                x.wv.vectors[x.wv.vocab[j].index] = model[j]
                sz += 1
        print(len(x.wv.vocab), sz)
...
#loadtrain()
print('load finish')
model = Word2Vec(corpus_file='input.txt', size=200, compute_loss=True, callbacks=(cbf(),), workers=1, iter=1, min_count=10)
model.save('zr')