...
path = 'D:\\ECNU\\ai\\z2-common sense\\8 Word Vector\\AWOE\\AWOE-master\\tmp\\keywords_aminer_en'
...
for i in range(len(model.vocab)):
    #print(model.index2word[i], model.vocab[model.index2word[i]].__dict__)
    sum = sum + len(model.index2word[i]) * model.vocab[model.index2word[i]].count
    word += model.vocab[model.index2word[i]].count
    minm = min(minm, model.vocab[model.index2word[i]].count)
print(sum)
print(minm)
print(word)

dic = set(model.index2word)
...
def loadh():
    with open('noword.txt','r',encoding='utf-8') as f:
        a = int(f.readline()[:-1])
        for i in range(a):
...
def fenci():
    ...
    with open('noword.txt','w',encoding='utf-8') as f:
        f.write(str(len(unk)))
        f.write('\n')
...
def loadtrain():
    with open('fenci+.txt','r',encoding='utf-8') as f:
        for i in range(10000):
            input.append(f.readline()[:-2].split('A'))
        f.close()
...
class cbf:
    def on_train_begin(self, x):
        self.cnte = 0
        print('weights init')
        xdic = set(x.wv.index2word)
        for i in dic:
            if(i in xdic):
                x.wv.vectors[x.wv.vocab[i].index] = model[i]
        print(xdic)
        print('begin training...')
    def on_epoch_begin(self, x):
        self.cnte += 1
        print('epoch begin', self.cnte)
...
#loadtrain()
print('load finish')
model = Word2Vec(corpus_file='fenci+.txt', size=200, compute_loss=True, callbacks=(cbf(),), workers=1, iter=50, negative=0, hs=1, sg=1)
print(Word2Vec.get_latest_training_loss())