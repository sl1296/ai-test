...
class cbf:
    def on_train_begin(self, x):
        print('call back on train begin', x)
    def on_epoch_begin(self, x):
        print('on epoch begin', x)
    def on_batch_begin(self, x):
        print('on batch begin', x)
    def on_batch_end(self, x):
        print('on batch end', x)
    def on_epoch_end(self, x):
        print('on_epoch_end', x)
    def on_train_end(self, x):
        print('on_train_end', x)
...
#loadtrain()
#print(input)
#model = Word2Vec(input, size=200, compute_loss=True, callbacks=(cbf(),), workers=1)
