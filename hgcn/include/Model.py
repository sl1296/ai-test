...
def get_mat(e, KG):
    ...
    for tri in KG:
        if tri[0] != tri[2]:
            du[tri[0]] += 1
            du[tri[2]] += 1
        if(tri[0] != tri[2] and ((tri[0], tri[2]) not in M)):
            M[(tri[0], tri[2])] = 1
            M[(tri[2], tri[0])] = 1
    for i in range(e):
        M[(i, i)] = 1
...
def build(dimension, act_func, gamma, k, lang, e, ILL, KG):
    ...
    input_layer = get_input_layer(e, dimension, lang)#词向量+L2norm
    M = get_sparse_tensor(e, KG)#GCN矩阵
    head, tail, head_r, tail_r = rfunc(KG, e)#计算每个关系相关的实体
...
def training(output_prel_e, output_joint_e, output_r, loss_1, loss_2, learning_rate, epochs, ILL, e, k, s, test, test_r, head, tail, fname):
    ...
    coinc=detect_coinc(test_r, head, tail, ILL, fname)
    print('train start')
        ...
        if i<s:  # preliminary training
            ...
            if i % 10 == 0:
                th, outvec_e, outvec_r = sess.run([loss_1, output_prel_e, output_r],
                                                feed_dict=feeddict)
                J.append(th)
                get_hits(outvec_e, test, fname)
                if i == s-1 or i==0:
                    get_hits_rel(outvec_r, test_r, coinc, fname)
