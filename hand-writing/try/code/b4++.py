    ...
    for xx in range(151800,500001):
        ...
        if xx%1 == 0:
            ...
            for z in range(1):
                ...
                acc3 = 0
                for i in item:
                    feature_encoder.load_state_dict(torch.load('b4-fe-252000.pkl'))
                    relation_network.load_state_dict(torch.load('b4-rn-252000.pkl'))
                    cc += 1
                    ...
                    if pre == i:
                        acc2 += 1
                    print(pre, acc2/cc,end='|| ')
                    feature_encoder.load_state_dict(torch.load('b1-fe-131000.pkl'))
                    relation_network.load_state_dict(torch.load('b1-rn-131000.pkl'))
                    samples = torch.zeros(50, 1, 84, 84)
                    for j in range(10):
                        for k in range(5):
                            samples[j*5+k] = transform(get_image(read_image(data[rmx[j]][test_data[rmx[j]][k]])))
