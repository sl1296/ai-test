...
def main():
    ...
    for xx in range(10001):
        ...
        acc = 0
        for i in range(A*2):
            if batch_labels[i][0] == 1 and relations[i][0] > 0.9:
                acc += 1
            if batch_labels[i][0] == 0 and relations[i][0] < 0.1:
                acc += 1
        mse = nn.MSELoss().cuda()
        ...
        if xx%200 == 0:
            torch.save(feature_encoder.state_dict(), 'b3-fe-'+str(xx)+'.pkl')
            torch.save(relation_network.state_dict(), 'b3-rn-'+str(xx)+'.pkl')
        if xx%1000 == 999:
            print("Testing...")
            accuracies = []
            '''
