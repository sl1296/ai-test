...
class Encode(nn.Module):
    def __init__(self, r_len):
        ...
        self.abc = nn.Sequential(
            nn.Linear(6*6*64, r_len),
            nn.Sigmoid(),
            nn.BatchNorm1d(r_len))

        self.den0 = nn.Sequential(
            nn.Linear(r_len, r_len),
            nn.Sigmoid(),
            nn.Softmax())
        
        self.cba = nn.Sequential(
            nn.Linear(r_len, 6*6*64),
            nn.Sigmoid())
...
def main():
    ...
    for xx in range(1001):
        ...
        for tx, ty in generate(train_img, size, step_train, r_len, words, False):
            ...
            print('\r%d/%d[' % (step, step_train)+'='*pos+'>'*min(1,30-pos)+'.'*(29-pos) + '] - ETA:%d:%02d - loss:%f lossx:%f lossy:%f acc:%f'%(int(eta/60), int(eta%60), closs/step, clossx/step, clossy/step, cacc/step) +' '*30, end='')
        now = time.time()
        print('\r%d/%d[' % (step, step_train)+'='*pos+'>'*min(1,30-pos)+'.'*(29-pos) + '] - %ds %dms/step - loss:%f lossx:%f lossy:%f acc:%f'%(round(now-pre), round((now-pre)*1000/step), closs/step, clossx/step, clossy/step, cacc/step) +' '*30)
