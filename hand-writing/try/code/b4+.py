...
def get_data(data, transform):
    ret = []
    if random.sample(range(15), 1)[0] < 100:
        c = random.sample(range(2000), A*2)
        b = random.sample(range(len(data[c[0]])), 2)
        ret = []
...
def main():
    ...
    feature_encoder.load_state_dict(torch.load('b4-fe-14800.pkl'))
    relation_network.load_state_dict(torch.load('b4-rn-14800.pkl'))
    '''
    ...
    for xx in range(14801,500001):
        ...
        relations = relation_network(relation_pairs).view(1, A*2)
        relations = F.softmax(relations)
        
        batch_labels = batch_labels.view(1, A*2).long().cuda()
        acc = 0
        for i in range(A*2):
            if batch_labels[0][i] == 1 and relations[0][i] > 0.99:
                acc += 1
            if batch_labels[0][i] == 0 and relations[0][i] < 0.01:
                acc += 1
        for i in range(A*2):
            if batch_labels[0][i] == 1:
                batch_labels = torch.tensor([[i]]).cuda()
                break
        print(batch_labels)
        print(relations)
        mse = nn.CrossEntropyLoss().cuda()
            ...
            for z in range(1):
                ...
                for i in item:
                    ...
                    for j in ret:
                        if j > ret[i]:
                            cnt += 1
                    print(i, ret.index(max(ret)), ret[i], cnt, acc/cc)
