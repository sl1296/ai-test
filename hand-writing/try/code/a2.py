...
class Encode(nn.Module):
    def __init__(self, r_len):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(96))
        self.maxp1 = nn.MaxPool2d(3,stride=2,padding=1)
        self.conv2 = nn.Sequential(
            nn.Conv2d(96, 128, 3, padding=1),
            nn.ReLU(),
            nn.BatchNorm2d(128))
        self.maxp2 = nn.MaxPool2d(3,stride=2,padding=1)
        self.conv3 = nn.Sequential(
            nn.Conv2d(128, 160, 3, padding=1),
            nn.ReLU(),
            nn.BatchNorm2d(160))
        self.maxp3 = nn.MaxPool2d(3,stride=2,padding=1)
        self.conv4 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(256))
        self.maxp4 = nn.MaxPool2d(3,stride=2,padding=1)
        self.conv5 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(384))
        self.maxp5 = nn.MaxPool2d(3,stride=2,padding=1)
        self.abc = nn.Sequential(
            nn.Linear(3*3*384, 1000),
            nn.ReLU(),
            nn.BatchNorm1d(1000))

        self.den0 = nn.Sequential(
            nn.Linear(1000, r_len),
            nn.Softmax())
        '''
        self.cba = nn.Sequential(
            nn.Linear(20, 6*6*60),
            nn.ReLU(),
            nn.BatchNorm1d(6*6*60))
        self.umaxp4 = nn.MaxUnpool2d(2)
        self.uconv4 = nn.Sequential(
            nn.ConvTranspose2d(60, 60, 3, padding=1),
            nn.BatchNorm2d(60),
            nn.ReLU())
        self.umaxp3 = nn.MaxUnpool2d(2)
        self.uconv3 = nn.Sequential(
            nn.ConvTranspose2d(60, 30, 3, padding=1),
            nn.BatchNorm2d(30),
            nn.ReLU())
        self.umaxp2 = nn.MaxUnpool2d(2)
        self.uconv2 = nn.Sequential(
            nn.ConvTranspose2d(30, 15, 3, padding=1),
            nn.BatchNorm2d(15),
            nn.ReLU())
        self.umaxp1 = nn.MaxUnpool2d(2)
        self.uconv1 = nn.Sequential(
            nn.ConvTranspose2d(15, 1, 3, padding=1),
            nn.Tanh())
        '''
    def forward(self, x):
        x = self.conv1(x)
...
def main():
    ...
    pretrained_dict = torch.load('a1-2.pkl')
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)
    #model.load_state_dict(torch.load('a2-2.pkl'))
    ...
    for xx in range(1001):
        ...
        for tx, ty in generate(train_img, size, step_train, r_len, words, False):
            ...
            ls = lossy(ry, y)
            #ls = lsx + lsy
            acc = (torch.argmax(ry, 1) == y).sum().item()/size
            opt.zero_grad()
            ls.backward()
            opt.step()
            #clossx += lsx.item()
            ...
            print('\r%d/%d[' % (step, step_train)+'='*pos+'>'*min(1,30-pos)+'.'*(29-pos) + '] - ETA:%d:%02d - loss:%f acc:%f'%(int(eta/60), int(eta%60), closs/step, cacc/step) +' '*30, end='')
        now = time.time()
        print('\r%d/%d[' % (step, step_train)+'='*pos+'>'*min(1,30-pos)+'.'*(29-pos) + '] - %ds %dms/step - loss:%f acc:%f'%(round(now-pre), round((now-pre)*1000/step), closs/step, cacc/step) +' '*30)
        print(datetime.datetime.now(), 'use:', time.time() - start)
        torch.save(model.state_dict(), 'a2-'+str(xx)+'.pkl')
