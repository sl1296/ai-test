...
def get_data(data, class_num, sample_num, batch_num, xx, transform):
    if xx != -1:
...
def main():
    ...
    for xx in range(10001):
        samples, batches, batch_labels = get_data(data, CLASS_NUM, SAMPLE_NUM, BATCH_NUM, xx, transform)
        ...
        if xx%200 == 199:
            ...
            class_num = 50
            for i in range(5):
                test_data = [random.sample(range(len(data[i])), SAMPLE_NUM+5) for i in range(r_len)]
                acc = 0
                for i in range(r_len):
                    ret = [0 for i in range(r_len)]
                    batches = torch.zeros(5, 1, 84, 84)
                    for z in range(5):
                        batches[z] = transform(get_image(read_image(data[i][test_data[i][-z-1]])))
                    for j in range(1001, r_len, class_num):
                        sz = min(class_num, r_len-j)
                        samples = torch.zeros(sz*SAMPLE_NUM, 1, 84, 84)
                        cc = 0
                        for k in range(j, j+sz):
                            for l in test_data[k][:-5]:
                                samples[cc] = transform(get_image(read_image(data[k][l])))
                                cc += 1

                        sample_features = feature_encoder(Variable(samples).cuda())
                        sample_features = sample_features.view(sz, SAMPLE_NUM, 64, 19, 19)
                        sample_features = torch.sum(sample_features, 1).squeeze(1)
                        batch_features = feature_encoder(Variable(batches).cuda())
                        sample_features_ext = sample_features.unsqueeze(0).repeat(5, 1, 1, 1, 1)
                        batch_features_ext = batch_features.unsqueeze(0).repeat(sz, 1, 1, 1, 1)
                        batch_features_ext = torch.transpose(batch_features_ext, 0, 1)
                        relation_pairs = torch.cat((sample_features_ext, batch_features_ext),2).view(-1, 128, 19, 19)
                        relations = relation_network(relation_pairs).view(-1, sz)

                        for k in range(j, j+sz):
                            ret[k] = relations.data[0][k-j]
                    if ret.index(max(ret)) == i:
                        acc += 1
                    print(i+1, acc, acc/(i+1))
