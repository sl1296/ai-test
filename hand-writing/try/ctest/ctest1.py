...
from keras.layers import PReLU, Flatten, Dense, Convolution2D, MaxPooling2D, Activation, Dropout, BatchNormalization
...
def get_cnn(r_len):
    model = Sequential()
    model.add(Convolution2D(100, (3, 3), padding='same', input_shape=(48, 48, 1)))
    model.add(PReLU())
    model.add(BatchNormalization())
    model.add(MaxPooling2D((3, 3), strides=1))
    model.add(Convolution2D(250, (3, 3), padding='same'))
    model.add(PReLU())
    model.add(BatchNormalization())
    model.add(MaxPooling2D((3, 3), strides=1))
    model.add(Convolution2D(350, (3, 3), padding='same'))
    model.add(PReLU())
    model.add(BatchNormalization())
    model.add(MaxPooling2D((3, 3), strides=1))
    model.add(Convolution2D(450, (3, 3), padding='same'))
    model.add(PReLU())
    model.add(BatchNormalization())
    model.add(MaxPooling2D((3, 3), strides=1))
    model.add(Flatten())
...
def get_den(r_len):
    ...
    model.add(Dense(units=50, activation='relu'))
    model.add(BatchNormalization())
    model.add(Dense(units=r_len, activation='relu'))
    model.add(Activation('softmax'))
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    return model
...
def main():
    ...
    model = get_cnn(r_len)
    load = False
    #model.load_weights('ctest1-1.h5')
    if load:
        #model.load_weights('cnn-FF-5.h5')
        img = cv2.imread('1.png',cv2.IMREAD_GRAYSCALE)
        img = cv2.threshold(img, 192, 1, cv2.THRESH_BINARY_INV)[1]
        '''
        print(img)
        plt.imshow(img)
        plt.show()
        '''
        ret = model.predict_classes(img[np.newaxis,:,:,np.newaxis])
        print(words[ret[0]])
