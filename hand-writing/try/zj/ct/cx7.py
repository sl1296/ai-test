...
def get_cnn(r_len):
    model = Sequential()
    model.add(Convolution2D(64, (3, 3), activation='relu', input_shape=(110, 110, 1)))  # 54*54
    model.add(MaxPooling2D((2, 2)))  # 52*52
    model.add(BatchNormalization())
    model.add(Convolution2D(96, (3, 3), activation='relu'))  # 54*54
    model.add(MaxPooling2D((2, 2)))  # 52*52
    model.add(BatchNormalization())
    model.add(Convolution2D(192, (3, 3), activation='relu'))  # 26*26
    model.add(MaxPooling2D((2, 2)))  # 24*24
    model.add(BatchNormalization())
    model.add(Convolution2D(384, (3, 3), activation='relu'))  # 12*12
...
def main():
    ...
    for xx in range(15,21):
        if xx>15:
            ...
            model.save_weights('cx7-' + str(xx) + '.h5')
