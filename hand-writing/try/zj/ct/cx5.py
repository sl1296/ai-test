...
def get_cnn(r_len):
    ...
    model.add(Convolution2D(64, (3, 3), activation='relu'))  # 54*54
    model.add(MaxPooling2D((2, 2)))  # 52*52
    model.add(BatchNormalization())
    model.add(Convolution2D(64, (3, 3), activation='relu'))  # 26*26
...
def main():
    ...
    for xx in range(21,23):
        #model.fit_generator(generate(train_img, size, step_train, r_len, words, False), steps_per_epoch=step_train, epochs=1)
        #model.save_weights('cx5-' + str(xx) + '.h5')
        #print('TR:',xx-2,datetime.datetime.now(), 'use:', time.time() - start)
        ...
        if xx % 1 == 0:
            ...
            print('TE:',xx-2,datetime.datetime.now(), 'use:', time.time() - start)
