...
def get_cnn(r_len):
    model = Sequential()
    model.add(Convolution2D(64, (3, 3), activation='selu', input_shape=(110, 110, 1)))  # 54*54
    model.add(MaxPooling2D((2, 2)))  # 52*52
    model.add(BatchNormalization())
    model.add(Convolution2D(96, (3, 3), activation='selu'))  # 54*54
    model.add(MaxPooling2D((2, 2)))  # 52*52
    model.add(BatchNormalization())
    model.add(Convolution2D(192, (3, 3), activation='selu'))  # 26*26
    model.add(MaxPooling2D((2, 2)))  # 24*24
    model.add(BatchNormalization())
    model.add(Convolution2D(384, (3, 3), activation='selu'))  # 12*12
...
def main():
    ...
    for xx in range(1,21):
        ...
        model.save_weights('cx8-' + str(xx) + '.h5')
