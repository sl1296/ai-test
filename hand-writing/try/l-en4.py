...
def get_den(r_len):
    ...
    model.add(Dense(units=300, activation='selu', kernel_initializer='lecun_normal'))
    model.add(Dense(units=200, activation='selu', kernel_initializer='lecun_normal'))
    model.add(Dense(units=100, activation='selu', kernel_initializer='lecun_normal'))
    model.add(Dense(units=75, activation='selu', kernel_initializer='lecun_normal'))
    model.add(Dense(units=50, activation='selu', kernel_initializer='lecun_normal'))
    model.add(Dense(units=r_len, activation='selu', kernel_initializer='lecun_normal'))
    model.add(Activation('softmax'))
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    return model
...
def main():
    ...
    for xx in range(0,1001):
        ...
        model.save_weights('len4-' + str(xx) + '.h5')
