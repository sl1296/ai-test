...
def get_cnn(r_len):
    model = Sequential()
    model.add(Convolution2D(96, (3, 3), padding='same', input_shape=(96, 96, 1)))
    model.add(PReLU())
    ...
    model.add(BatchNormalization())
    model.add(MaxPooling2D((3, 3), strides=2))
    model.add(Convolution2D(160, (3, 3), padding='same'))
    model.add(PReLU())
    model.add(BatchNormalization())
    model.add(MaxPooling2D((3, 3), strides=2))
...
def main():
    ...
    for xx in range(1,1001):
        ...
        model.save_weights('ctest-' + str(xx) + '.h5')
