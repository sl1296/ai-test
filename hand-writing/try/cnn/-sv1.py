...
def generate(data, size, step, r_len, add, words, load = True):
    if load:
        while True:
            random.shuffle(data)
            for i in range(step):
        ...
        while True:
            ...
            for i in range(step):
                sz = min(size, len(data[0]) - i * size)
                #for j in range(sz):
                #    show_image(data[0][j],data[1][j],words)
                yield data[0][i*size:i*size+sz,:,:,:], data[1][i*size:i*size+sz,:]
...
def main():
    ...
    score = model.evaluate_generator(generate(train_img, size, step_train, r_len, False, words), steps=step_train)
    print('Train Loss:', score[0])
    print('Train Accuracy:', score[1])
    score = model.evaluate_generator(generate(test_img, size, step_test, r_len, False, words), steps=step_test)
